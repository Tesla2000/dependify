...
def injected(class_: Type, *, container: Container = _container):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for arg, (field_name, field_type) in zip(args, annotations_iter):
            if not isinstance(arg, field_type):
                raise TypeError(f"Expected {field_type} for {field_name}, got {type(arg)}")
        ...
        for field_name, value in tuple(kwargs_copy.items()):
            if field_name not in class_.__annotations__:
                raise TypeError(f"Keyword argument: {field_name} not found in class {class_.__name__}")
            if field_name not in missing_args:
                raise TypeError(f"Keyword argument: {field_name} already provided as a positional argument")
            field_type = class_.__annotations__[field_name]
            if not isinstance(value, field_type):
                raise TypeError(f"Expected {field_type} for {field_name}, got {type(value)}")
    ...
    __init__.__signature__ = Signature(tuple(Parameter(name, Parameter.POSITIONAL_OR_KEYWORD, annotation=annotation) for name, annotation in __init__.__annotations__.items()))
